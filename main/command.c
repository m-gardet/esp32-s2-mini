...
#define LOAD_CONFIG_NAME "CONFIG LOAD"
...
int cmd_load_config(uint8_t* cmd, uint8_t len);
...
int usb_printf(char* buffer, ...)
{
  ...
  if(len>0)
  {
    tinyusb_cdcacm_write_queue(TINYUSB_USBDEV_0, pOutputBuffer, len);
...
{
  //gpio_dump_io_configuration(stdout, pin);
...
int load_config(void)
{
  ...
  if(err = ESP_OK)
  {
    ...
    if(err == ESP_OK)
    {
      ESP_LOGI(TAG, "Reading config form false:");
  ...
  {
    //usb_printf("sys_name[0] 0x%X\n\r", config_flash.sys_name[0]);
    //usb_printf("integrity_name %s\n\r", config_flash.integrity_name);
    //ESP_LOG_BUFFER_HEXDUMP(TAG, &config_flash, sizeof(config), ESP_LOG_INFO);
...
int cmd_save_config(uint8_t* cmd, uint8_t len)
{
  ...
  // Write blob
  if(nvs_set_blob(my_handle, "config", (void*)&config, sizeof(config)) != ESP_OK)
  {
    usb_printf(CMD_KO "flash Update error\n\r");
    nvs_close(my_handle);
    return -1;
